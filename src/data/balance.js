...
export const BASE_TUNING = {
  ...
  tension: {
    fillTimeAt1: 0.90,      // при ratio=1.0 (вес/прочность) за сколько сек заполнится 0→100
    curveAlpha: 1.20,       // кривизна скорости роста при тяжёлых рыбинах
    downPerSec: 150,        // спад при отпускании (ед/сек)
  },

  // Скорости перемещения по воде
  speeds: {
    basePull: 1500,          // базовая скорость подтягивания игроком (px/s) до модификаторов
    baseFish: 7 ,           // базовая «уплывалка» рыбы (px/s) до модификаторов
    minPull: 30,            // нижний предел, чтобы не было «залипания»
    minFish: 18,            // ← опусти, если хочешь ОЧЕНЬ медленную рыбу
  ...
  escape: {
    base: 3.2,
    byHookQ: 1.4,           // чем лучше подсечка — тем дольше окно
    byHookCtrl: 0.5,        // хорошие крючки дают люфт
